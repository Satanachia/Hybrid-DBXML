namespace XMLDB3
{
    ...
    public class CharacterItemDeleteCommand : SerializedCommand
    {
        ...
        protected override bool _DoProces()
        {
            ...
            WorkSession.WriteStatus("CharacterItemDeleteCommand.DoProcess() : [" + this.m_ID + "] 의 데이터를 캐쉬에서 읽습니다");
            CharacterInfo info = (CharacterInfo) ObjectCache.Character.Extract(this.m_ID);
            WorkSession.WriteStatus("CharacterItemDeleteCommand.DoProcess() : [" + this.m_ID + "] 의 아이템을 삭제합니다.");
            this.m_Result = QueryManager.Character.DeleteItem(this.m_ID, this.m_ItemList);
            if (this.m_Result)
            {
                WorkSession.WriteStatus("CharacterItemDeleteCommand.DoProcess() : [" + this.m_ID + "] 의 아이템을 삭제하였습니다");
                if (((info != null) && (this.m_ItemList != null)) && (this.m_ItemList.Length > 0))
                {
            ...
            {
                WorkSession.WriteStatus("CharacterItemDeleteCommand.DoProcess() : [" + this.m_ID + "] 의 아이템을 삭제에 실패하였습니다");
            }
