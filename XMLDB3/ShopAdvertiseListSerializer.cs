    ...
    public class ShopAdvertiseListSerializer
    {
        public static Message Deserialize(ShopAdvertiseList _list, Message _message)
        {
            if (((_list != null) && (_list.advertises != null)) && (_list.advertises.Length > 0))
            {
                ...
                foreach (ShopAdvertiseDetail detail in _list.advertises)
                {
                    WriteShopAdveritise(detail, _message);
                }
                return _message;
            }
            _message.WriteS32(0);
        ...
        private static Message WriteShopAdveritise(ShopAdvertiseDetail _advertise, Message _message)
        {
            ...
            {
                _message.WriteS32(_advertise.items.Length);
                foreach (ShopAdvertiseItemDetail detail in _advertise.items)
                {
                    WriteItem(detail, _message);
                }
                return _message;
            }
            _message.WriteS32(0);
